...
out = ((NPV(obj, call_flag, S + 0.005, X, T, v, r, q) - ...
    NPV(obj, call_flag, S - 0.005, X, T, v, r, q)) ./ (2 * 0.005));
end

function out = VEGA(obj, call_flag, S, X, T, v, r, q)
% Compute Vega using two-sided finite difference method
out = ((NPV(obj, call_flag, S, X, T, v + .005, r, q) - ...
    NPV(obj, call_flag, S, X, T, v - .005, r, q)) ./ (2 * 0.005));
end
