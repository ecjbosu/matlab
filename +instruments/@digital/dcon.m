function out = dcon(obj, param)
% dcon - Cash or Nothing Digital Option Price formula
% based on Zhang 2nd Ed.  Pager 400 - 402
...
% npv is the core pricer calculation function
    ...
    if ~isempty(indx)
        ...
        out(indx) = exp(-R(indx)  .* T(indx)) .* normcdf(type(indx) .* d2(indx));
    end
