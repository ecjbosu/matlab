function sendmail(to,subject,theMessage, subtype, attachments)
...
%
%   matpacks.iofun.sendmail_html(myaddress, 'mail Test', 'This is a test message.', [], 'c:\temp\temp.txt');
%   matpacks.iofun.sendmail_html(myaddress, 'mail Test', 'This is a test message.', 'html', 'c:\temp\temp.txt');
%   HTML emails are generated by setting the sendmail preference flag for
%   html to 'html'. If it does not exist then it defaults to text emails
%
%   See also WEB, FTP.
...
% $Revision: 1.4.4.19 $  $Date: 2011/12/16 16:33:24 $
% Modified by: Joe W. Byers, Ph.D.

% This function requires Java.
if ~usejava('jvm')
   error(message('matpacks:sendmail:NoJvm'));
end
...
%added by Joe W. Byers for html body
if nargin < 4 || isempty(subtype)
    subtype = [];
else
    if ~ismember(subtype, {'html'})
        subtype = 'html';
    end
end
if (nargin < 5) 
...
if isempty(server)
    ...
    error(message('matpacks:sendmail:SMTPServerIndeterminate',commandStr));
end
if isempty(from)
    commandStr = 'setpref(''Internet'',''E_mail'',''username@example.com'');';
    error(message('matpacks:sendmail:FromAddressIndeterminate',commandStr));
end
...
%Joe Byers added for html emails
...
if any(subject == char(10)) || any(subject == char(13))
    error(message('matpacks:sendmail:InvalidSubject'));
end
if isJapanese
...
if numel(attachments) == 0
    if isJapanese
        ...
        if ~isempty(subtype)
            msg.setText(body, charset, subtype);
        else
    ...
    if isJapanese
        ...
        if ~isempty(subtype)
            messageBodyPart.setText(body, charset, subtype);
        else
    ...
    for iAttachments = 1:numel(attachments)
        ...
        if isempty(fullName)
            error(message('matpacks:sendmail:CannotOpenFile', file));
        end
...
try
    ...
    if isempty(niceError)
        ...
        error(message('matpacks:sendmail:SmtpError', niceError))
    end
end
...
try
    ...
    error(message('matpacks:sendmail:AddressError', stripJavaError( exception.message )));
end
...
%html call add markup to cell arrays
...
if ~isempty(m)
    switch m{1}
        case 'javax.mail.AuthenticationFailedException'
            niceError = getString(message('matpacks:sendmail:assignment_AuthenticationFailed'));
            return
    end
end
