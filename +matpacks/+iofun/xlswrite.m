...
%   $Revision: 1.1.6.23 $  $Date: 2011/02/15 00:53:55 $
%==============================================================================
% Set default values.
Sheet1 = 1;
...
try
    ...
    if ~isempty(file)
        if ~ischar(file)
            error('matpacks:xlswrite:InputClass','Filename must be a string.');
        end
        % check for wildcards in filename
        if any(strfind(file, '*'))
            error(message('matpacks:xlswrite:FileName'));
        end
        ...
        if a1 && ~(a2.UserWrite == 1)
            error(message('matpacks:xlswrite:FileReadOnly'));
        end
    else % get workbook filename.
        error(message('matpacks:xlswrite:EmptyFileName'));
    end

    % Check for empty input data
    if isempty(data)
        error(message('matpacks:xlswrite:EmptyInput'));
    end

    % Check for N-D array input data
    if ndims(data)>2
        error(message('matpacks:xlswrite:InputDimension'));
    end

    % Check class of input data
    if ~(iscell(data) || isnumeric(data) || ischar(data)) && ~islogical(data)
        error('matpacks:xlswrite:InputClass',...
...
try
    ...
    set(Excel, 'UserControl', false);

catch exception %#ok<NASGU>
    warning('matpacks:xlswrite:NoCOMServer',...
    ...
    try
        ...
        exceptionNew = MException('matpacks:xlswrite:dlmwrite', 'An error occurred on data export in CSV format.');
...
try
    ...
    function ExecuteWrite
        ...
        if ExcelWorkbook.ReadOnly ~= 0
            ...
            error(message('matpacks:xlswrite:LockedFile', file));
        end
        try % select region.
            ...
            throw(MException('matpacks:xlswrite:SelectDataRange', sprintf('Excel returned: %s.', exceptionInner.message))); 
        end
...
try
    ...
    warning(message('matpacks:xlswrite:AddSheet'));
    if nargout > 0
        [theMessage.message,theMessage.identifier] = lastwarn;
    end
end
...
try
    ...
    error(message('matpacks:xlswrite:CalculateRange', range));
end
end
%----------------------------------------------------------------------
...
if d ~= floor(d) || any(d(:) < 0) || any(d(:) > 1/eps)
    error(message('matpacks:xlswrite:Dec2BaseInput'));
end
